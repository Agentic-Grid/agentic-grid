...
export function RenameDialog({
  ...
  // Initialize with currentName since component mounts fresh each time
  const [name, setName] = useState(currentName);
  const [error, setError] = useState<string | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);

  // Focus input and select filename on mount
  useEffect(() => {
          ...
          <div className="p-6">
            ...
            <div className="flex items-start gap-4">
              <div className="flex-shrink-0 p-2.5 rounded-xl glass border border-[var(--border-subtle)]">
                <IconRename className="w-6 h-6 text-[var(--accent-primary)]" />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="mt-1 text-sm text-[var(--text-tertiary)] truncate">
                  {currentPath}
                </p>
              </div>
            </div>
            ...
            {name && name !== currentName && (
              <div className="mt-4 p-3 rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-subtle)]">
                <p className="text-xs text-[var(--text-tertiary)] mb-1">New path:</p>
